...
dataset_names = list(adata.uns["spatial"].keys())
...
for pair_list in [sorted_top_pairs_tuple_all, extra_pairs]:
    ...
    for dataset_name in dataset_names:
        ...
        _, bandwidth_df = li.ut.query_bandwidth(
            coordinates=adata.obsm["spatial"], start=0, end=600, interval_n=20
        )
...
for count, slide in enumerate([ref_sample, fibr_sample]):
    ...
    condition = adata_sub.obs[condition_var][0]
    cond_test = "fibrosis" if condition == "IPF" else "reference"
